...
- **`ConditionalOrder.condition_type` deserialization** — replaced `#[serde(alias)]` with a custom `Deserialize` implementation that accepts `conditionType` (camelCase), `type` (platform Prisma column name), and `condition_type` (snake_case) without failing when multiple key formats appear in the same payload. The custom deserializer validates JSON types strictly (rejects non-string values for string fields, rejects unknown `status` variants) while `Serialize` continues to use the canonical `conditionType` key via derive. Prevents a compatibility regression during API transitions where the platform may switch from `conditionType` to `type`. (closes #250)
