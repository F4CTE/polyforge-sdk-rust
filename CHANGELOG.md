...
- **UpdateProfileParams.twitter_handle** — add optional `twitter_handle: Option<String>` field to the profile/me update params. Serializes as `twitterHandle` (camelCase). Matches `UpdateSettingsProfileParams` which already carries this field. Downstream struct-literal callers need to include `twitter_handle: None` or use `..Default::default()`. (closes #255)
