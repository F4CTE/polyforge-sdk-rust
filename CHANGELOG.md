...
- **`ConditionalOrder.condition_type` serde mapping** — add `#[serde(rename = "type", alias = "conditionType")]` to the public `condition_type` field so the SDK correctly maps the platform's `type` JSON field without breaking downstream consumers that reference `co.condition_type`. Previously the struct-level `camelCase` rename caused the field to expect `"conditionType"`, but the platform returns `"type"`. (closes #250)
