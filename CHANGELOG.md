...
- **UpdateProfileParams: mark `#[non_exhaustive]`** — the struct is now `#[non_exhaustive]` so that adding new optional fields in the future does not break downstream code. This is a deliberate semver signal: callers must use `..Default::default()` when constructing `UpdateProfileParams` literals (all fields are already `Option<T>` and `Default`-derived). Driven by Codex/Argus review feedback on PR #278.
...
- **UpdateProfileParams.twitter_handle** — add optional `twitter_handle: Option<String>` field to the profile update params struct. Serializes as `twitterHandle` (camelCase) to match the platform's `UpdateProfileDto`. All fields remain `Option<T>` with `Default` derived; callers using `..Default::default()` spread are unaffected. (closes #255)
