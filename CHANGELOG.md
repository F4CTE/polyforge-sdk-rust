...
- **`search_markets` response shape** — the platform returns `{ results: [...] }` but the SDK was trying to decode it as `PaginatedResponse` (`{ data, total, page, … }`), causing deserialization failures on every successful HTTP 200. Now deserialized internally as `SearchResults<Market>` and converted to `PaginatedResponse<Market>` so the public API stays backward-compatible. (closes #253)
