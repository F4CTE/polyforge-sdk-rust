...
### Fixed
- **`RiskSettings` / `UpdateRiskSettingsParams` field names** — both structs now use the platform's actual DTO field names (`drawdownEnabled`, `drawdownLookbackHours`, `drawdownThresholdPct`, `circuitBreakerTripped`, `circuitBreakerTrippedAt`) instead of the previous incorrect fields (`dailyLossLimit`, `maxPositionSize`, `maxBetsPerDay`, `circuitBreakerTriggered`). Every `PATCH /api/v1/settings/risk` call was returning 400 because the old field names did not match the platform's `forbidNonWhitelisted` validation. (closes #248)
