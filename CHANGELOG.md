...
- **Accuracy leaderboard (POLA-4833, #279)** — `get_accuracy_leaderboard(Option<&AccuracyLeaderboardParams>)` wraps `GET /api/v1/accuracy/leaderboard`, returning a paginated list of traders ranked by prediction accuracy with profile metadata (rank, username, display name, avatar, P&L, win rate, trade count). Supports `period`, `limit`, `page`, `offset` (auto-converted to page), and `cursor` parameters. New types: `AccuracyLeaderboardParams`, `AccuracyLeaderboardEntry`. Closes gap with `polyforge-sdk-ts` `getAccuracyLeaderboard()`.
