...
### Added
- **`CreateStrategyParams.kalshi_subaccount`** — adds an optional `kalshi_subaccount: Option<u64>` field to `CreateStrategyParams`, serialized as `kalshiSubaccount` (camelCase) and omitted when `None`. Enables passing a Kalshi subaccount identifier when creating strategies. `CreateStrategyParams` is now `#[non_exhaustive]` and has a `new(name)` constructor to ensure forward-compatible construction. (closes #4178)
