...
- **Fix `RiskSettings` / `UpdateRiskSettingsParams` field names (POLA-4161)** — replaced incorrect `dailyLossLimit`/`maxPositionSize`/`maxBetsPerDay`/`circuitBreakerTriggered` fields with the platform's actual schema: `drawdownEnabled`, `drawdownLookbackHours`, `drawdownThresholdPct`, `circuitBreakerTripped`, `circuitBreakerTrippedAt` (closes #248).
