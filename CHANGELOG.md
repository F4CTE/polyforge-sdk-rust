...
### Fixed
- **`RiskSettings` / `UpdateRiskSettingsParams` field names** — replace incorrect fields (`dailyLossLimit`, `maxPositionSize`, `maxBetsPerDay`, `circuitBreakerTriggered`) with the platform's actual DTO fields (`drawdownEnabled`, `drawdownLookbackHours`, `drawdownThresholdPct`, `circuitBreakerTripped`, `circuitBreakerTrippedAt`). `PATCH /api/v1/settings/risk` was returning 400 because old field names failed `forbidNonWhitelisted` validation.
