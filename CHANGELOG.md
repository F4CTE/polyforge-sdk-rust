...
- **UpdateProfileParams** — add `#[serde(flatten)] extra: HashMap<String, serde_json::Value>` for forward-compatible field extensions without further struct shape changes. New `UpdateProfileParamsBuilder` with convenience methods (`display_name()`, `bio()`, `avatar_url()`, `twitter_handle()`) and `UpdateProfileParams::builder()` / `UpdateProfileParams::new()` constructors. The `twitter_handle` setter serializes as `twitterHandle` (camelCase) to match the platform's `PATCH /api/v1/profile/me` contract and reach feature parity with `polyforge-sdk-python` and `polyforge-mcp`. Existing struct-literal constructions (`UpdateProfileParams { ..Default::default() }`) remain source-compatible. (closes #255)
