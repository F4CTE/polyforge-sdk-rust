...
### Fixed
- **`RiskSettings` response type mismatch** — completely rewrote `RiskSettings` and `UpdateRiskSettingsParams` to match the platform's actual contract. The old fields (`daily_loss_limit`, `max_position_size`, `max_bets_per_day`, `circuit_breaker_triggered`) never mapped to the real API response, causing every `get_risk_settings()`, `update_risk_settings()`, and `reset_circuit_breaker()` call to fail with serde deserialization errors. New fields: `drawdown_enabled`, `drawdown_lookback_hours`, `drawdown_threshold_pct`, `circuit_breaker_tripped`, `circuit_breaker_tripped_at`. Added `extra` flatten bucket for forward compatibility with server-side fields (`userId`, `updatedAt`). Added client-side validation mirroring the platform DTO's `@Min`/`@Max` rules: `drawdown_threshold_pct` (0.01–0.99) and `drawdown_lookback_hours` (1–168). `RiskSettings::default()` is now a manual impl consistent with serde empty-JSON deserialization. 11 unit tests verify full shape, minimal defaults, server-side fields, Default-vs-serde parity, validation rejections, and update params serialization. (closes #249)
