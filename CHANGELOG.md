...
- **UpdateProfileParams.twitter_handle** — add optional `twitter_handle: Option<String>` field to `UpdateProfileParams`, mirroring the earlier `UpdateSettingsProfileParams` addition. Serializes as `twitterHandle` (camelCase) to match the platform's `PATCH /api/v1/profile/me` contract and reach feature parity with `polyforge-sdk-python` and `polyforge-mcp`. (closes #255)
