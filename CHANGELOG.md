...
- **BREAKING `search_markets` response shape** — return type changed from `PaginatedResponse<Market>` to new `SearchResults<Market>` (`{ results: [...] }`), matching the actual platform response. Previously the SDK attempted to decode the `{ results }` payload as a `PaginatedResponse` which has `data`/`total`/`page`/`limit`/`totalPages`/`hasNext` fields, causing deserialization failures. *Migration:* `SearchResults<Market>` implements `Into<PaginatedResponse<Market>>` — call `.into()` on the result if you need the previous paginated shape. (closes #253)
