...
impl PolyforgeClient {
    ...
    ///
    /// Returns the updated risk settings with `circuitBreakerTripped: false`.
...
mod tests {
    ...
    fn test_risk_settings_deserializes_minimal() {
        ...
        let rs: RiskSettings = serde_json::from_str(json).unwrap();
        assert!(!rs.drawdown_enabled);
        assert_eq!(rs.drawdown_lookback_hours, 0);
        assert!((rs.drawdown_threshold_pct - 0.0).abs() < f64::EPSILON);
        assert!(!rs.circuit_breaker_tripped);
        assert!(rs.circuit_breaker_tripped_at.is_none());
    }
    ...
    fn test_risk_settings_circuit_breaker_tripped() {
        let json = r#"{
            ...
            "circuitBreakerTrippedAt": "2025-01-15T08:00:00Z"
        }"#;
        let rs: RiskSettings = serde_json::from_str(json).unwrap();
        assert!(rs.circuit_breaker_tripped);
        assert_eq!(rs.circuit_breaker_tripped_at.unwrap(), "2025-01-15T08:00:00Z");
    }
